...
Clone the git repositories for mag and gitian and then checkout the mag version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/mag-crypto/mag.git
cd mag
...
Follow the instructions in [doc/release-process.md](release-process.md) in the mag repository
...
gitian builds' in [doc/release-process.md](release-process.md) in the mag repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/mag/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/mag-crypto/mag
...
For example:
```bash
URL=https://github.com/crowning-/mag.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit mag=${COMMIT} --url mag=${URL} ../mag/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit mag=${COMMIT} --url mag=${URL} ../mag/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit mag=${COMMIT} --url mag=${URL} ../mag/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/mag-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/mag-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/mag-build.assert
```
...
[mag/gitian.sigs](https://github.com/mag-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
